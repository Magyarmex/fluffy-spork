 ...
 * AI perception mirrors the information already exposed to the player:
...
if(!mods){console.error('[NOVA v1.10.5] module registry unavailable');return;}

var VERSION='1.10.5',CODENAME='Shared Battlefield View',TAU=Math.PI*2;
...
function saturation(g,targetId,selfId){
  ...
  for(var i=0;i<ts.length;i++){
    ...
    var id=q.ai.__v1105TargetId>=0?q.ai.__v1105TargetId:(q.ai.__v180TargetId>=0?q.ai.__v180TargetId:q.ai.targetId);
...
function scoreTarget(g,classes,t,a,q,now){
  ...
  if(a.__v1105TargetId===q.id)score+=.24;
...
function movementPlan(g,classes,C,t,a,target,sight,risk,out){
  ...
  var side=a.__v1105FlankSide||a.__v180FlankSide||((((t.id||0)*17+target.id)&1)?1:-1);
...
function planAbility(g,classes,C,t,a,target,sight,risk,now){
  ...
  if(ab==='bulwark'||ab==='taunt')due=(risk.risk>.34)||(sight&&target&&dist<420&&hp<.72);
    ...
    else if(sight&&target&&target.hp<target.maxHp*.32&&dist<440){ang+=(a.__v1105FlankSide||1)*.34;due=true;}
  }
...
function rememberPublicTarget(a,target,now){
  a.__v1105TargetId=target.id;a.__v1105TrackX=target.x;a.__v1105TrackY=target.y;a.__v1105TrackVx=target.vx||0;a.__v1105TrackVy=target.vy||0;a.__v1105TrackAt=now;
...
function plan(t,g,classes,C,a,now){
  ...
  if(!target){
    a.__v1105TargetId=-1;a.__v180TargetId=-1;a.__v180Seen=false;a.__v180OwnMove=false;a.__v180AbilityPermit=false;
    if(a.state!=='flee')a.targetId=-1;
    a.__v1105PlanT=reactionFor(t,a);return;
  }
  ...
  a.__v1105PlanT=reactionFor(t,a);window.__NOVA_SHARED_BATTLEFIELD_VIEW__.plans++;
}
function controllerPlan(g,classes,owner,now){
  ...
  owner.ai.targetId=target.id;owner.ai.__v1105TargetId=target.id;if(owner.ai.state!=='flee')owner.ai.state='hunt';
...
wrap('game/ai',function(ai,require){
  ...
  function patched(t,g,dt){
    ...
    a.__v1105PlanT=(a.__v1105PlanT==null?0:a.__v1105PlanT)-dt;
    if(a.__v1105PlanT<=0)plan(t,g,classes,C,a,now);

    var target=a.__v1105TargetId>=0&&g.getTank?g.getTank(a.__v1105TargetId):null;
    ...
    target=a.__v1105TargetId>=0&&g.getTank?g.getTank(a.__v1105TargetId):null;
    if(target&&alive(target)){
      rememberPublicTarget(a,target,now);
      a.targetId=target.id;if(a.state!=='flee')a.state='hunt';
