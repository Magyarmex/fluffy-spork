 ...
 * Full Blackglass visual parity pass. The historical v1.7.3 portrait-fit layer
 * remains immutable and loads first; this release owns only the current mirror
 * renderer, weapon animation, visual muzzle geometry and canonical parity data.
 */
(function(){
'use strict';
...
var TAU=Math.PI*2;
var CSS=`
/* Preserve v1.7.3 portrait proportions while allowing the canonical mirror to
   own the visible simulation canvas. */
...
html.nvs-portrait-mobile .nvs-stagehint{z-index:4!important;}
...
@media(max-width:380px){.nvs-canvaswrap:has(.nvs-parity-canvas)::after{font-size:5.8px;left:7px;top:7px;}}
`;
...
function installMirrorStyle(){
  var style=document.getElementById(PARITY_ID);
  if(!style){style=document.createElement('style');style.id=PARITY_ID;style.textContent=CSS;document.head.appendChild(style);}
  else if(style.textContent!==CSS)style.textContent=CSS;
  var fit=document.getElementById(FIT_ID);
  if(fit&&style.previousElementSibling!==fit)document.head.appendChild(style);
}

var req=window.__novaMakeRequire,classes=null,CLASSES={};
try{classes=req&&req('blackglass-mirror-v1106')('./game/classes');CLASSES=classes&&classes.CLASSES||{};}catch(e){console.error('[NOVA v1.10.6] canonical class registry unavailable',e);}
...
function wake(){installMirrorStyle();ensureCanvas();if(visible()&&!state.raf)state.raf=requestAnimationFrame(loop);}
...
var headObs=new MutationObserver(function(m){for(var i=0;i<m.length;i++){var n=m[i].addedNodes||[];for(var j=0;j<n.length;j++){var id=n[j]&&n[j].id;if(id==='nova-showroom-fit-v173'||id==='nova-showroom-containment-v172'){installMirrorStyle();break;}}}});headObs.observe(document.head,{childList:true});
