/* NOVA TANKS v1.8.2 — Contact Spark
...
var VERSION='1.8.2',CODENAME='Contact Spark',TAU=Math.PI*2;
...
function ensureState(g){return g.__v182Contact||(g.__v182Contact={volleys:[],sparks:[],lastSwoosh:-99,nextId:1});}
function markVolley(g,t,bullets){
 ...
 if(!off)return;
 var tracked=[];for(var i=0;i<bullets.length;i++){var b=bullets[i];if(!b||b.dead||b.__novaRail||b.beam)continue;b.__v182Volley=state.nextId;b.__novaPrecisionFlyby=true;tracked.push(b);}
...
function scanEntries(g,dt){
 var state=g.__v182Contact;if(!state||!state.volleys.length)return;var now=g.time||0,out={x:0,y:0,t:0};
...
wrap('game/render',function(renderMod){
 ...
 function patched(g,w,h){window.__NOVA_CONTACT_SPARK_RENDERING__=true;try{oldRender(g,w,h);}finally{window.__NOVA_CONTACT_SPARK_RENDERING__=false;}if(!g||!g.ctx||!g.player||!g.player.alive||!g.__v182Contact)return;var state=g.__v182Contact,now=g.time||0,ctx=g.ctx;ctx.save();ctx.setTransform(g.dpr||1,0,0,g.dpr||1,0,0);for(var i=state.sparks.length-1;i>=0;i--){var s=state.sparks[i];if(!s||now>s.until){state.sparks.splice(i,1);continue;}drawSpark(ctx,s,now);}ctx.restore();}
