...
async function syncLatest(registration: ServiceWorkerRegistration): Promise<unknown> {
  ...
  syncPromise = (async () => {
    try {
      ...
      // Installed NOVA clients are expected to remain usable while offline.
    }
...
async function registerPeriodicUpdate(registration: ServiceWorkerRegistration): Promise<void> {
  ...
  try {
    ...
    await periodicRegistration.periodicSync.register('nova-update', {
      minInterval: 6 * 60 * 60 * 1000,
    });
  } catch {
    // Periodic Background Sync is browser-controlled and optional.
  }
}
  ...
  try {
    ...
    navigator.serviceWorker.addEventListener('message', (event: MessageEvent<unknown>) => {
      ...
      try {
        ...
        // Storage is optional in private/sandboxed contexts.
      }
    });
