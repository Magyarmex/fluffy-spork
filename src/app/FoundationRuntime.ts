...
export class FoundationRuntime implements UIApplicationPort {
  ...
  private resetTransientInput(){this.#pressed.clear();this.#pointerDown=false;this.#gamepadWasActive=false;for(const channel of Object.keys(this.#touchChannels) as TouchChannel[])this.#touchChannels[channel]=false;}
  private readonly onKeyDown=(event:KeyboardEvent)=>{this.#pressed.add(event.code);};private readonly onKeyUp=(event:KeyboardEvent)=>{this.#pressed.delete(event.code);};private readonly onPointerMove=(event:PointerEvent)=>{if(event.pointerType!=='touch')this.#pointerClient={x:event.clientX,y:event.clientY};};private readonly onPointerDown=(event:PointerEvent)=>{if(event.pointerType==='touch')return;this.#pointerDown=event.button===0;this.#pointerClient={x:event.clientX,y:event.clientY};};private readonly onPointerUp=(event:PointerEvent)=>{if(event.pointerType!=='touch')this.#pointerDown=false;};private readonly onBlur=()=>{this.resetTransientInput();};private readonly onVisibilityChange=()=>{if(document.hidden)this.resetTransientInput();};
  private installInput(){window.addEventListener('keydown',this.onKeyDown);window.addEventListener('keyup',this.onKeyUp);window.addEventListener('blur',this.onBlur);document.addEventListener('visibilitychange',this.onVisibilityChange);this.#presenter.canvas.addEventListener('pointermove',this.onPointerMove);this.#presenter.canvas.addEventListener('pointerdown',this.onPointerDown);window.addEventListener('pointerup',this.onPointerUp);window.addEventListener('pointercancel',this.onPointerUp);}
  private removeInput(){window.removeEventListener('keydown',this.onKeyDown);window.removeEventListener('keyup',this.onKeyUp);window.removeEventListener('blur',this.onBlur);document.removeEventListener('visibilitychange',this.onVisibilityChange);this.#presenter.canvas.removeEventListener('pointermove',this.onPointerMove);this.#presenter.canvas.removeEventListener('pointerdown',this.onPointerDown);window.removeEventListener('pointerup',this.onPointerUp);window.removeEventListener('pointercancel',this.onPointerUp);}
}
