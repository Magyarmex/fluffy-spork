...
const source = fs.readFileSync(path.join(__dirname, '../../nova-updates/lobby-battlefield-v1.9.4.js'), 'utf8');

function metadataBoot() {
  const window = {};
  const context = { window, document: undefined, navigator: {}, console, Math };
  vm.runInNewContext(source, context, { filename: 'lobby-battlefield-v1.9.4.js' });
  return window;
}
...
test('War Room ships the complete 36-tank roster at level 30', () => {
  ...
  assert.equal(window.__NOVA_VERSION, '1.9.4');
