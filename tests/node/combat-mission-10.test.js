...
function loadCanonicalCombat() {
  ...
  const sources = [...tsFiles(path.join(root, 'src', 'content')), path.join(root, 'src', 'game', 'simulation', 'math.ts'), ...tsFiles(combatDir)];
  ...
  return { combat: require(path.join(outDir, 'game', 'combat', 'index.js')), content: require(path.join(outDir, 'content', 'index.js')), dispose: () => rmSync(outDir, { recursive: true, force: true }) };
}
...
test('Mission 10 consumes Mission 05 weapon definitions across every lineage', () => {
  ...
  try {
    for (const tankId of ['scout', 'twin', 'cannon', 'marksman', 'carrier', 'guard']) {
      const weapon = content.WeaponRegistry.get(`${tankId}:weapon`);
      const result = fire(new combat.CombatSystem(), weapon, actor(tankId));
...
test('Twin alternates barrels, shotgun preserves pellet cone, and twin-beam damage is split', () => {
  ...
  try {
    ...
    const twin = content.WeaponRegistry.get('twin:weapon');
    ...
    const shotgun = content.WeaponRegistry.get('shotgun:weapon');
    ...
    const prism = content.WeaponRegistry.get('prism:weapon');
...
test('Cooldowns, minigun spool and inherited shooter velocity remain deterministic', () => {
  const { combat, content, dispose } = loadCanonicalCombat();
  try {
    const weapon = content.WeaponRegistry.get('minigun:weapon');
    ...
    assert.equal(fire(system, weapon, shooter, first.cooldownRemainingSeconds / 2, { fireSpin: 1 }).fired, false);
...
test('Direct hit owns damage, armor, penetration, destruction and friendly-fire rejection', () => {
  ...
  try {
    const system = new combat.CombatSystem();
    const shot = fire(system, content.WeaponRegistry.get('marksman:weapon'), actor('marksman'), 0).projectiles[0];
    const hit = system.resolveDirectHit({ projectile: shot, target: actor('target', 'red', { health: 20, maxHealth: 100, armor: 2 }), atSeconds: 0.1 });
    ...
    assert.equal(system.resolveDirectHit({ projectile: shot, target: actor('friend', 'blue'), atSeconds: 0.1 }).appliedDamage, 0);
  } finally { dispose(); }
});
...
test('Cannon splash honors cover exposure and canonical cover damage uses the battlefield port', () => {
  ...
  try {
    const system = new combat.CombatSystem();
    const projectile = fire(system, content.WeaponRegistry.get('cannon:weapon'), actor('cannon'), 0).projectiles[0];
    const splash = system.resolveSplash(projectile, [{ combatant: actor('open', 'red'), exposure: 1 }, { combatant: actor('edge', 'red'), exposure: 0.4 }, { combatant: actor('blocked', 'red'), exposure: 0 }], 0.2);
...
test('Cluster shells deterministically emit the canonical child count without recursive clusters', () => {
  ...
  try {
    const system = new combat.CombatSystem();
    const weapon = content.WeaponRegistry.get('bomber:weapon');
    const children = system.spawnCluster(fire(system, weapon, actor('bomber'), 0).projectiles[0], { x: 10, y: 20 }, (i) => `child-${i}`);
...
test('Ultimate abilities reproduce live Ragnarok, Point Blank, Supercharge and Phase semantics', () => {
  ...
  try {
    const system = new combat.CombatSystem();
    const cannonWeapon = content.WeaponRegistry.get('demolisher:weapon');
    const ragnarok = system.activateAbility({ actor: actor('cannon'), ability: content.AbilityRegistry.get('ragnarok'), weapon: cannonWeapon, aimRadians: 0, atSeconds: 0, projectileId: (i) => `rag-${i}` });
    ...
    const shotgunWeapon = content.WeaponRegistry.get('shotgun:weapon');
    const pointblank = system.activateAbility({ actor: actor('shotgun'), ability: content.AbilityRegistry.get('pointblank'), weapon: shotgunWeapon, aimRadians: 0, atSeconds: 0, projectileId: (i) => `pb-${i}` });
    ...
    sniperSystem.activateAbility({ actor: sniper, ability: content.AbilityRegistry.get('supercharge'), atSeconds: 0 });
    const rail = content.WeaponRegistry.get('railgun:weapon');
    const charged = fire(sniperSystem, rail, sniper, 0).projectiles[0];
    assert.equal(charged.damage, rail.projectile.damage * 2.5);
    assert.equal(charged.penetrationRemaining, Infinity);

    const phase = system.activateAbility({ actor: actor('ghost', 'blue', { position: { x: 2200, y: 0 } }), ability: content.AbilityRegistry.get('phase'), aimRadians: 0, atSeconds: 0 });
...
test('Controller and Guardian ultimates expose authoritative timed combat actions and defenses', () => {
  ...
  try {
    const system = new combat.CombatSystem();
    const swarm = system.activateAbility({ actor: actor('controller'), ability: content.AbilityRegistry.get('swarm'), atSeconds: 0 });
    ...
    system.activateAbility({ actor: guardian, ability: content.AbilityRegistry.get('bulwark'), atSeconds: 0 });
    const incoming = fire(new combat.CombatSystem(), content.WeaponRegistry.get('scout:weapon'), actor('enemy', 'red'), 0).projectiles[0];
    ...
    iron.activateAbility({ actor: guardian, ability: content.AbilityRegistry.get('taunt'), atSeconds: 0 });
    ...
    const stampede = new combat.CombatSystem().activateAbility({ actor: guardian, ability: content.AbilityRegistry.get('stampede'), atSeconds: 0 });
    assert.equal(stampede.effects[0].moveSpeedMultiplier, 1.9);
    assert.equal(stampede.effects[0].bodyDamageMultiplier, 2);
  } finally { dispose(); }
});
