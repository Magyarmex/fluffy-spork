...
const source = fs.readFileSync(path.join(__dirname, '../../nova-updates/upgrade-dwell-v1.9.1.js'), 'utf8');
...
function boot() {
  ...
  vm.runInNewContext(source, context, { filename: 'upgrade-dwell-v1.9.1.js' });
  return { window, React, timers, cleared, getState: () => hookState };
}

test('release installs a strict 500 ms upgrade dwell', () => {
  const { window } = boot();
  assert.equal(window.__NOVA_VERSION, '1.9.1');
