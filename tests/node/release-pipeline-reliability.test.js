...
test('remaining releases preserve chronological wrapper order while Live Vector supersedes Command Weave',()=>{const names=['visual-overhaul-v1.9.0.js','sensory-feedback-v1.9.1.js','upgrade-dwell-v1.9.2.js','spotter-comms-v1.9.3.js','lobby-battlefield-v1.10.1.js','terrain-intelligence-v1.10.2.js','drone-field-service-v1.10.3.js','shared-battlefield-view-v1.10.5.js','second-body-live-vector-v1.10.7.js'];let prev=-1;for(const name of names){const at=workflow.indexOf(name);assert.ok(at>prev,name+' missing or out of order');prev=at;}assert.equal(workflow.indexOf("'./nova-updates/controller-command-weave-v1.10.0.js'"),-1,'retired Command Weave must not be materialized');});
test('new completion layers are explicitly materialized',()=>{for(const name of ['sensory-feedback-v1.9.1.js','drone-field-service-v1.10.3.js','second-body-live-vector-v1.10.7.js'])assert.ok(workflow.split(name).length-1>=2,name+' must appear in injection and verification');});
