...
function loadShared(){
  ...
  const shared=fs.readFileSync(path.join(__dirname,'../../nova-updates/shared-battlefield-view-v1.10.5.js'),'utf8');
  vm.runInNewContext(predator,context,{filename:'predator-doctrine-v1.8.0.js'});
  vm.runInNewContext(shared,context,{filename:'shared-battlefield-view-v1.10.5.js'});
  const cache={};
  function load(id){
...
test('v1.10.5 declares player-map information parity without granting physical cheats',()=>{
  const {context,src}=loadShared();
  const c=context.window.__NOVA_SHARED_BATTLEFIELD_VIEW__.contract;
  assert.equal(context.window.__NOVA_SHARED_BATTLEFIELD_VIEW_RELEASE__.version,'1.10.5');
...
test('cover no longer freezes or expires a live target coordinate',()=>{
  ...
  assert.equal(hunter.ai.__v1105TargetId,7);
  player.x=910;player.y=705;player.vx=55;player.vy=-30;
  g.blockAll=true;g.time+=.2;hunter.ai.__v1105PlanT=0;
  ai.updateAI(hunter,g,.16);
  assert.equal(hunter.ai.__v1105TargetId,7);
...
test('map awareness has no tank-centric vision radius',()=>{
  ...
  g.tanks=[hunter,player];g.player=player;
  ai.updateAI(hunter,g,.16);
  assert.equal(hunter.ai.__v1105TargetId,7,'a living tank shown on the minimap remains knowable across the arena');
...
test('shared awareness preserves a bounded reaction cadence',()=>{
  ...
  assert.ok(hunter.ai.__v1105PlanT>.07,'planning should remain reaction-limited rather than frame-perfect');
});
