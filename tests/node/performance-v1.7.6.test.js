...
function loadLayer(){
  ...
  const src=fs.readFileSync(path.join(__dirname,'../../nova-updates/performance-v1.7.6.js'),'utf8');
  vm.runInNewContext(src,context,{filename:'performance-v1.7.6.js'});
  const cache={};
...
test('Zero Churn publishes v1.7.6',()=>{
  const {context}=loadLayer();
  assert.equal(context.window.__NOVA_PERFORMANCE_2_RELEASE__.version,'1.7.6');
