...
function load(){
  ...
  vm.runInNewContext(fs.readFileSync(path.join(__dirname,'../../nova-updates/precision-contact-v1.8.2.js'),'utf8'),context);
  const cache={};
...
test('offscreen hostile precision volley gets one contact spark and one swoosh',()=>{
  ...
  assert.ok(g.__v182Contact);
  assert.equal(g.__v182Contact.volleys.length,1);
  assert.equal(g.bullets[0].__novaPrecisionFlyby,true,'legacy flyby audio is ceded to entry swoosh');
  g.updateBullets(1.3);
  assert.equal(g.__v182Contact.sparks.length,1);
