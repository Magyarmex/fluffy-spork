...
const source = fs.readFileSync(path.join(root, 'nova-updates/visual-language-v1.10.8.js'), 'utf8');
...
function boot() {
  ...
  vm.runInNewContext(source, context, { filename: 'visual-language-v1.10.8.js' });
  return window;
}
...
test('production materializer loads Signal Discipline after Live Vector and verifies it', () => {
  assert.match(deploy, /'\.\/nova-updates\/second-body-live-vector-v1\.10\.7\.js',\n\s*'\.\/nova-updates\/visual-language-v1\.10\.8\.js'/);
  assert.match(deploy, /grep -q 'nova-updates\/visual-language-v1\.10\.8\.js'/);
});

test('future production runtime files that emit visuals must declare NOVA_VISUAL_INTENT', () => {
  const scripts = [...deploy.matchAll(/'\.\/nova-updates\/([^']+\.js)'/g)].map(m => m[1]);
  const boundary = scripts.indexOf('second-body-live-vector-v1.10.7.js');
