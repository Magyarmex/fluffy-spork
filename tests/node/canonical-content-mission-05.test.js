...
function load(){const out=mkdtempSync(path.join(tmpdir(),'nova-content-final-'));const tsc=require.resolve('typescript/bin/tsc');execFileSync(process.execPath,[tsc,'--target','ES2022','--module','commonjs','--moduleResolution','node','--skipLibCheck','--strict','--rootDir',path.join(root,'src'),'--outDir',out,path.join(root,'src/content/index.ts')],{cwd:root,stdio:'pipe'});return{api:require(path.join(out,'content/index.js')),dispose:()=>rmSync(out,{recursive:true,force:true})};}
const loaded=load();test.after(()=>loaded.dispose());const api=loaded.api;

test('canonical content registry is self-contained after frozen legacy specimen retirement',()=>{assert.equal(api.TankRegistry.size,36);assert.equal(api.WeaponRegistry.size,36);assert.equal(api.DroneRegistry.size,36);assert.equal(api.BattlefieldRegistry.size,3);assert.equal(api.UpgradeRegistry.size,8);for(const id of ['scout','twin','cannon','marksman','carrier','guard','tempest','annihilator','prism','hivemind','bastion'])assert.ok(api.TankRegistry.has(id),id);});
