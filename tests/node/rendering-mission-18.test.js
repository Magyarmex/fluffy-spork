...
missionTest('Mission 18 runtime battlefield snapshot does not visually resurrect destroyed cover', () => { const renderer=new api.rendering.BattlefieldRenderer(), geometry={shape:'rect',x:10,y:20,width:100,height:40}, state={bounds:{minX:-500,maxX:500,minY:-400,maxY:400},terrain:[{id:1,type:'cover',geometry,destructible:true,maxHealth:300,health:0,solid:false,brokenAtMs:1000},{id:2,type:'wall',geometry:{shape:'circle',x:-40,y:0,radius:30},destructible:false,maxHealth:0,health:0,solid:true,brokenAtMs:null}],rubble:[{terrainId:1,geometry,createdAtMs:1000}]}, commands=renderer.build(api.content.BattlefieldRegistry.get('crossfire'),state);assert.equal(commands[0].kind,'rect');assert.equal(commands[0].width,1000);assert.equal(commands.filter(c=>c.layer==='terrain').length,2,'one solid wall plus one rubble scar; destroyed cover must not remain solid');assert.ok(commands.some(c=>c.layer==='terrain'&&c.alpha===.42)); });
