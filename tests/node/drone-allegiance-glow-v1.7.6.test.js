...
test('IFF Halo publishes a reinforced but still secondary visual-only release',()=>{const {context}=loadLayer();const r=context.window.__NOVA_DRONE_IFF_RELEASE__,t=context.window.__NOVA_DRONE_IFF_TEST__;assert.equal(r.version,'1.7.6');assert.equal(r.codename,'IFF Halo');assert.ok(t.alpha>=0.16&&t.alpha<=0.2);assert.ok(t.coreAlpha>t.alpha&&t.coreAlpha<0.4);});
test('owned drones draw two blue passes while hostile drones draw two red passes',()=>{const h=loadLayer();const owners=[{id:1,alive:true},{id:2,alive:true}],drones=[{ownerId:1,x:-20,y:0,hp:10,role:'escort'},{ownerId:2,x:20,y:0,hp:10,role:'hunter'}];h.render.render(game(h.ctx,owners,drones),200,120);assert.equal(h.getBaseRenders(),1);assert.deepEqual(h.draws.map(d=>d.color),['#4da8ff','#4da8ff','#ff4d62','#ff4d62']);assert.deepEqual(h.draws.map(d=>d.alpha),[0.18,0.30,0.18,0.30]);assert.ok(h.draws[0].w>h.draws[1].w);});
test('same-team drones are friendly and different-team drones are hostile',()=>{const h=loadLayer(),player={id:1,alive:true,teamId:'A'},owners=[player,{id:2,alive:true,teamId:'A'},{id:3,alive:true,teamId:'B'}],drones=[{ownerId:2,x:-15,y:0,hp:10,role:'escort'},{ownerId:3,x:15,y:0,hp:10,role:'escort'}];h.render.render(game(h.ctx,owners,drones,player),200,120);assert.deepEqual(h.draws.map(d=>d.color),['#4da8ff','#4da8ff','#ff4d62','#ff4d62']);});
test('non-hostile relation hooks stay neutral instead of being painted red',()=>{const h=loadLayer(),player={id:1,alive:true},owners=[player,{id:2,alive:true}],g=game(h.ctx,owners,[{ownerId:2,x:0,y:0,hp:10,role:'escort'}],player);g.areHostile=()=>false;h.render.render(g,200,120);assert.equal(h.draws.length,0);});
test('off-screen drones are culled before both IFF passes',()=>{const h=loadLayer(),owners=[{id:1,alive:true},{id:2,alive:true}],drones=[{ownerId:1,x:0,y:0,hp:10,role:'escort'},{ownerId:2,x:5000,y:5000,hp:10,role:'hunter'}];h.render.render(game(h.ctx,owners,drones),200,120);assert.equal(h.draws.length,2);assert.ok(h.draws.every(d=>d.color==='#4da8ff'));});
