...
function loadReliability(){
  ...
  const src=fs.readFileSync(path.join(__dirname,'../../nova-updates/reliability-v1.8.2.js'),'utf8');
  vm.runInNewContext(src,context,{filename:'reliability-v1.8.2.js'});
...
test('Integrity Pass publishes v1.8.2 as presentation-only reliability work',()=>{
  const {context,src}=loadReliability();
  assert.equal(context.window.__NOVA_VERSION,'1.8.2');
  assert.equal(context.window.__NOVA_RELIABILITY_RELEASE__.version,'1.8.2');
